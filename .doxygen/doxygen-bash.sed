...
# Handle comment blocks delimited by # followed by 50+ dashes.
# Collect inner lines, convert # to ##, then jump to the @fn handler
# (via :step) or handle declare/plain cases directly.
/^# -\{50,\} *$/{
    s/.*/##/
    :dashblock
    N
    /\n# -\{50,\} *$/{
        s/\n# -\{50,\} *$/\n##/
        # Convert inner # to ## using a loop to handle all lines
        :dash2hash
        s/\(\n\)#\( \|\n\)/\1##\2/
        t dash2hash
        s/\(\n\)#$/\1##/
        # If block contains @fn, jump to :step so the @fn handler
        # reads ahead to find the function definition line.
        /@fn/b step
        # Otherwise read the next line (declare or other)
        N
        /\ndeclare [^\n]*$/{
            h
            s/\n[^\n]*$//
        ...
        # For any other following line, convert ## to //! and print
        s/\(^\|\n\)##\n/\1\/\/!\n/g
        s/\(^\|\n\)## /\1\/\/! /g
        s/\(^\|\n\)##$/\1\/\/!/g
        p
